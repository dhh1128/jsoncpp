...
struct ValueTest : JsonTest::TestCase {
    ...
    ValueTest()
        : emptyArray_(json::vt_array)
        , emptyObject_(json::vt_object)
        , integer_(123456789)
...
std::string ValueTest::normalize_floating_point_str(std::string const& s)
{
    ...
    if (index != std::string::npos) {
        ...
        std::string::size_type index_digit = s.find_first_not_of('0', exponent_start_index);
        std::string exponent = "0";
        if (index_digit != std::string::npos) // There is an exponent different from 0
        {
...
JSONTEST_FIXTURE(ReaderTest, parseWithOneError)
{
    ...
    bool ok = reader.parse("{ \"property\" :: \"value\" }", root);
    JSONTEST_ASSERT(!ok);
    JSONTEST_ASSERT(reader.get_formatted_messages() == "* Line 1, Column 15\n  Syntax error: value, object or array "
                                                       "expected.\n");
    std::vector<json::reader::structured_error> errors = reader.get_structured_errors();
    JSONTEST_ASSERT(errors.size() == 1);
    JSONTEST_ASSERT(errors.at(0).offset_start == 14);
    JSONTEST_ASSERT(errors.at(0).offset_limit == 15);
    JSONTEST_ASSERT(errors.at(0).message == "Syntax error: value, object or array expected.");
}
...
JSONTEST_FIXTURE(ReaderTest, parseChineseWithOneError)
{
    ...
    JSONTEST_ASSERT(!ok);
    JSONTEST_ASSERT(reader.get_formatted_messages() == "* Line 1, Column 19\n  Syntax error: value, object or array "
                                                       "expected.\n");
    std::vector<json::reader::structured_error> errors = reader.get_structured_errors();
    JSONTEST_ASSERT(errors.size() == 1);
...
JSONTEST_FIXTURE(ReaderTest, parseWithDetailError)
{
    ...
    JSONTEST_ASSERT(!ok);
    JSONTEST_ASSERT(reader.get_formatted_messages() == "* Line 1, Column 16\n  Bad escape sequence in string\nSee "
...
JSONTEST_FIXTURE(CharReaderTest, parseWithOneError)
{
    ...
    bool ok = reader->parse(
        ...
        &root, &errs);
    JSONTEST_ASSERT(!ok);
    JSONTEST_ASSERT(errs == "* Line 1, Column 15\n  Syntax error: value, object or array "
                            "expected.\n");
    delete reader;
}
...
JSONTEST_FIXTURE(CharReaderTest, parseChineseWithOneError)
{
    ...
    bool ok = reader->parse(
        ...
        &root, &errs);
    JSONTEST_ASSERT(!ok);
    JSONTEST_ASSERT(errs == "* Line 1, Column 19\n  Syntax error: value, object or array "
                            "expected.\n");
    delete reader;
}
...
JSONTEST_FIXTURE(CharReaderTest, parseWithDetailError)
{
    ...
    bool ok = reader->parse(
        ...
        &root, &errs);
    JSONTEST_ASSERT(!ok);
    JSONTEST_ASSERT(errs == "* Line 1, Column 16\n  Bad escape sequence in string\nSee "
                            "Line 1, Column 20 for detail.\n");
    delete reader;
}
...
JSONTEST_FIXTURE(CharReaderStrictModeTest, dupKeys)
{
    json::char_reader_builder b;
    json::value root;
    char const doc[] = "{ \"property\" : \"value\", \"key\" : \"val1\", \"key\" : \"val2\" }";
    {
